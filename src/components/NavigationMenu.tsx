            ...
            return (
              <Link
                ...
                className={`flex items-start gap-3 p-4 transition-colors ${
                  active ? 'bg-primary/10 text-primary-foreground' : 'hover:bg-muted/60'
                }`}
              >
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${active ? 'gradient-primary text-primary-foreground' : 'bg-muted text-foreground'}`}>
                  <Icon className="w-5 h-5" />
                </div>
                <div className="flex-1">
                  <p className={`font-medium ${active ? 'text-primary-foreground' : 'text-foreground'}`}>
                    {t(labelKey)}
                  </p>
