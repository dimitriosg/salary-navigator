...
import { Calculator, ArrowRightLeft, Building2, Users } from 'lucide-react';
  ...
  return (
    <Card className="w-full max-w-lg mx-auto">
      <CardHeader className="text-center pb-2">
        ...
        <CardTitle className="text-2xl">Κόστος Εργοδότη</CardTitle>
        <CardDescription>Υπολογίστε το συνολικό κόστος για τον εργοδότη ανά εργαζόμενο</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="employer-gross">Μηνιαίος Μικτός Μισθός (€)</Label>
          <Input
            id="employer-gross"
            type="number"
          ...
          <div className="mt-6 space-y-4 animate-slide-up">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-lg bg-accent/10 border border-accent/20">
                <p className="text-sm text-muted-foreground mb-1">Κόστος Εργοδότη</p>
            ...
            <div className="p-4 rounded-lg bg-secondary/10">
              <p className="text-sm text-muted-foreground mb-1">Καθαρά στον Εργαζόμενο</p>
            ...
            <div className="border-t border-border pt-4">
              <h4 className="font-semibold text-foreground mb-3">Ανάλυση Εισφορών</h4>
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center py-2 px-3 rounded-lg bg-accent/5 border border-accent/10">
                  <span className="text-sm text-muted-foreground">ΕΦΚΑ Εργοδότη (21.79%)</span>
                  <span className="font-medium text-accent-foreground">{formatCurrency(result.efkaEmployer)}</span>
                </div>
                <div className="flex justify-between items-center py-2 px-3 rounded-lg bg-muted/50">
            ...
            {/* Summary comparison */}
            <div className="border-t border-border pt-4">
              <h4 className="font-semibold text-foreground mb-3">Σύγκριση</h4>
              <div className="space-y-2">
                <div className="flex justify-between items-center py-2 px-3 rounded-lg bg-muted/50">
                  <span className="text-sm text-muted-foreground">Συνολικές Εισφορές</span>
                  <span className="font-medium text-foreground">
                    {formatCurrency(result.efkaEmployee + result.efkaEmployer)}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 px-3 rounded-lg bg-muted/50">
                  <span className="text-sm text-muted-foreground">% Κόστους σε Εισφορές</span>
                  <span className="font-medium text-foreground">
                    {(((result.efkaEmployee + result.efkaEmployer) / result.totalEmployerCost) * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 px-3 rounded-lg bg-muted/50">
                  <span className="text-sm text-muted-foreground">% Κόστους σε Καθαρά</span>
            ...
            <div className="text-xs text-muted-foreground mt-4 p-3 bg-muted/30 rounded-lg">
              ...
              <p>* Εργοδοτικές εισφορές ΕΦΚΑ: 21.79%</p>
            </div>
          </div>
        )}
      </CardContent>
